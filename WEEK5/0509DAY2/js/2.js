...
box.onclick = function (ev) {
    ...
    // ev.pageX / ev.pageY：当前鼠标触发点距离BODY(第一屏幕)左上角的X/Y轴坐标   //body有时会因为滚动条的scroll变化
    // ev.preventDefault()：阻止默认行为            ////在Event的实例上，不是直接在MouseEvent的实例上
    // ev.stopPropagation()：阻止事件的冒泡传播     ////在Event的实例上，不是直接在MouseEvent的实例上
    // ev.type：当前事件类型                       ////在Event的实例上，不是直接在MouseEvent的实例上
...
tempInp.onkeydown = function (ev) {
    console.log('onkeydown_ev',ev);
    console.log('onkeydown_ev.which',ev.which);
};
