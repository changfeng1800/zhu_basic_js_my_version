...
$(function () {
    var $magnifierBox = $('.magnifierBox'),
        $smallBox = $magnifierBox.find('.smallBox'),    ////find后代元素的好方法
        $mark = $smallBox.find('.mark'),
        $bigBox = $magnifierBox.find('.bigBox'),
        $bigImg = $bigBox.find('img');

    //=>控制MARK和BIG-BOX的显示隐藏
    $smallBox.on('mouseenter', function (ev) {  ////on方法是用DOM2级事件绑定
    ...
    function computedMark(ev) {     ////jQuery的ev不存在兼容问题
        var offsetObj = $smallBox.offset(),
            curL = ev.pageX - offsetObj.left - $mark.outerWidth() / 2, ////计算mark的左边框到magnifierBox左边的距离
