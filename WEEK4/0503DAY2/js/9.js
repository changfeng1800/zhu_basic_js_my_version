$(function () {
    ...
    ~function () {
        ...
        $container.html(str);   ////=== innerHtml

        $imgList = $container.find('img');  ////找到所有的img
    }();
    ...
    let lazyImg = curImg => {
        ...
        tempImg.onload = () => {
            ...
            $curImg.attr('src', trueImg)
                .stop().fadeIn(300);//=>结束当前正在运行的动画,执行FADE-IN,让图片300MS内渐现出来(JQ中提供的动画方法),////stop所有的动画，再fadeIn
            tempImg = null;     ////释放内存
    ...
    let computedImg = () => {
        ...
        $imgList.each((index, curImg) => {
            ...
            //=>B:当前浏览器底边距离BODY偏移
            let $curImg = $(curImg),
                $imgBox = $curImg.parent(), ////img被div包起来的
