...
//     let res = callback(10, 20); ////实参在这里传
//     console.log(res);
// };
// fn((n, m) => {  ////函数执行，传参，参数是一个回调函数
...
////准备自己写个jQuery的each方法
//$.each()是和内置的FOR-EACH类似（但是FOR-EACH只能遍历数组），用来遍历数组（类、数组、对象）中的每一项的
//$.each();

// [12, 23, 34].forEach(function (item, index) {
//     console.log(item, index, this);  ///回调函数里面的this
//     //=>this:window
// });
...
//             if (res === false) {    ////如果return false就不再for循环了
//                 break;
//             }
//         }
//     } else {
...
String.prototype.myReplace = function myReplace(reg, callback) {
    ...
    let res = reg.exec(this),   ////结果是数据，没匹配到是 null
        _this = this;
    while (res) {   ////没匹配到是 null
