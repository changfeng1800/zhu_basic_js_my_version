...
xhr.onreadystatechange = () => {
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {   ////xhr.status === 0才能得到数据
        val = xhr.responseText;
        console.log('val1',val);//=>如果使用异步AJAX请求，不等AJAX彻底完成，就把VAL输出，此时的结果是NULL
        //=>此处是获取结果,获取结果后还要做很多的事情（此时我们只能把数数绑定等任务写在这里）
    }
};
xhr.send(null);
console.log('val2',val);//=>如果使用异步AJAX请求，不等AJAX彻底完成，就把VAL输出，此时的结果是NULL */
