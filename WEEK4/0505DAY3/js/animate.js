...
~function () {
    ...
    let utils = (function () {
        ...
        let setCss = (ele, attr, value) => {
            if (!isNaN(value)) {
                if (!/^(opacity|zIndex)$/.test(attr)) { ////非opacity|zIndex加单位，其它不加
                    value += 'px';
                }
            }
        ...
        let css = (...arg) => {
            let len = arg.length,
                fn = getCss;
            if (len >= 3) { ////有3个参数，是设置一个css样式
                fn = setCss;
            }
            if (len === 2 && typeof arg[1] === 'object') {  ////有2个参数，同时第2个参数是object
                fn = setGroupCss;
            }
            return fn(...arg);  ////写法真他妈精简
        };
    ...
    window.animate = function (ele, target = {}, duration = 1000, callback = new Function()) {
        ...
        ele.animteTimer = setInterval(() => {
            ...
            if (time >= duration) {
                ...
                callback.call(ele);//=>动画完成后执行CALL-BACK(并且让回调函数中的THIS是当前操作的元素本身)  ////牛
                return;
            }
