...
~function () {
    class Banner {
        constructor(options = {}) { ////new的时候constructor函数会执行

            //=>OPTIONS传递的配置项(解构赋值并且给更多的配置项设置默认值)
            let {
                ele,
                url,
                isArrow = true, ////设置默认值，有默认值的不传也可以
                isFocus = true,
                isAuto = true,
                defaultIndex = 0,
                interval = 3000,
            ...
            ['ele', 'url', 'isArrow', 'isFocus', 'isAuto', 'defaultIndex', 'interval', 'speed', 'moveEnd'].forEach(item => {
                this[item] = eval(item);    ////实例上绑定一个属性，值是一个变量噢
            });

            this.container = document.querySelector(ele);   ////这些也绑定到实例上，方便调用
        ...
        bindHTML() {
            ...
            data.forEach((item, index) => {
                let {img = 'img/banner1.jpg', desc = '珠峰'} = item;
