let bannerRender = (function () {
    ...
    let queryData = function queryData() {
        return new Promise((resolve, reject) => {
            ...
            xhr.onreadystatechange = () => {
                ...
                if (xhr.readyState === 4 && xhr.status === 0) {   ////老师写200，我这里却xhr.status === 0才可以
    ...
    let bindHTML = function bindHTML(data) {
        ...
        data.forEach((item, index) => {
            ...
            let {img = 'img/banner1.jpg', desc = '珠峰'} = item;    ////这个没有active ???
        ...
        //->获取所有的SLIDE和LI
        slideList = wrapper.querySelectorAll('.slide');     ////querySelectorAll不会有DOM映射
    ...
    let handleFocus = function handleFocus() {
        [].forEach.call(focusList, (item, index) => {
            item.onclick = function () {
                stepIndex = index;//=>点击的是谁，就让STEP-INDEX运动到哪（STEP-INDEX和点击LI的索引一致即可）    ////这里有forEach中会形成块级作用域吗
                animate(wrapper, {
                    left: -stepIndex * 1000
                }, 200);
                changeFocus();
